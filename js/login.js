...
(function () {
  ...
  var champOubli = document.getElementById('forgot-email');
  if (champOubli) {
    champOubli.addEventListener('input', function () { efface(champOubli, 'forgot-error'); });
  }
  ...
  document.getElementById('forgot-form').addEventListener('submit', function (event) {
    ...
    /* Le seul refus muet qui restait sur cette page. Le formulaire porte
       novalidate : sans message, on cliquait « Envoyer », le curseur sautait
       dans le champ, et rien ne disait pourquoi. */
    if (!email.value.trim()) {
      return manque(email, 'forgot-error', 'Indiquez l\'adresse de votre compte.');
    }
    if (!email.checkValidity()) {
      return manque(email, 'forgot-error',
        '« ' + email.value.trim() + ' » n\'est pas une adresse email.');
    }
