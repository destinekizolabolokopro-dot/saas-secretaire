...
(function () {
  ...
  function build() {
    ...
    /* Les sections de Mon compte viennent de l'espace pro, pas d'une seconde
       liste écrite ici : les deux avaient divergé, et la palette ignorait
       Préférences, Notifications et Support — dont la première est justement
       celle où vivent la déconnexion et les réglages du quotidien. */
    (hooks.sections || []).forEach(function (section) {
      list.push({
        kind: 'Aller à', label: section.label,
        run: function () { hooks.goAccount(section.id); }
      });
    });

    if (store.can('voiceCommand')) {
      list.push({ kind: 'Action', label: 'Parler à Ally', run: function () { hooks.openVoice(); } });
    }
