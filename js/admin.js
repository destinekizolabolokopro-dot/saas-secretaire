...
(function () {
  ...
  function bindDrawer(user) {
    ...
    /* Deux temps avant ce qui coupe l'accès de quelqu'un.

       « Suspendre » se faisait d'un clic, dans une liste, et refuse ensuite la
       connexion au cabinet : du point de vue du client, c'est aussi brutal
       qu'une suppression, et bien plus facile à déclencher par erreur.
       « Supprimer » passait par window.confirm — une boîte du navigateur,
       étrangère au produit, que certains contextes bloquent purement et
       simplement, auquel cas le clic ne faisait rien du tout.

       Le premier clic arme et nomme la conséquence, le second exécute, et
       l'armement retombe seul au bout de cinq secondes. C'est le geste déjà
       employé pour la base de connaissances et pour le retrait d'un
       collaborateur. */
    function deuxTemps(button, libelleArme, faire) {
      ...
      button.addEventListener('click', function () {
        if (!button.getAttribute('data-armed')) {
          el.drawerBody.querySelectorAll('[data-act][data-armed]').forEach(function (autre) {
            if (autre !== button) autre.dispatchEvent(new CustomEvent('admin-desarme'));
          });
          button.setAttribute('data-armed', '1');
          button.textContent = libelleArme;
      ...
      button.addEventListener('admin-desarme', desarmer);
    }
    ...
    el.drawerBody.querySelectorAll('[data-act]').forEach(function (button) {
      ...
      if (act === 'suspend') {
        deuxTemps(button, 'Confirmer — ce cabinet ne pourra plus se connecter', function () {
      ...
      if (act === 'delete') {
        deuxTemps(button, 'Confirmer la suppression définitive', function () {
          /* remove() inscrit déjà « deleted » au journal : le refaire ici
             donnerait deux lignes pour une seule suppression. */
  ...
  function bindPanel() {
    ...
    /* Même geste en deux temps que dans la fiche d'un compte : une boîte du
       navigateur est étrangère au produit, et certains contextes la bloquent —
       le clic ne faisait alors rien du tout. */
    var reseed = document.getElementById('sys-reseed');
    if (reseed) armerPuisFaire(reseed, 'Confirmer — les comptes créés depuis seront perdus', doReseed);
  }

  /* Version autonome du geste en deux temps, pour les boutons qui ne vivent
     pas dans le tiroir d'une fiche. */
  function armerPuisFaire(button, libelleArme, faire) {
    var libelle = button.textContent;
    var minuteur = null;
