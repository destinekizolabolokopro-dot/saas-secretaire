...
(function () {
  ...
  window.ALLY_BRAIN = {
    ...
    ask: function (input) {
      ...
      if (hasAny(t, ['ajoute', 'cale', 'creneau', 'nouveau rendez'])) {
        var nd = findDay(t), nt = findTime(t);
        return {
          kind: 'action', sensitive: true,
          confirm: 'Je pose un rendez-vous' + (nd ? ' ' + nd : '') + (nt ? ' à ' + nt.replace(':', 'h') : '')
            + ', vous confirmez ?',
          reply: 'C\'est noté' + (nd ? ', ' + nd : '') + (nt ? ' à ' + nt.replace(':', 'h') : '') + '.',
          detail: 'Créneau vérifié contre vos disponibilités'
        };
      }
