...
(function () {
  ...
  var ACTIONS = {
    today: [
      ...
      /* « Résumé envoyé » était faux : aucun service d'envoi n'est branché,
         et le dire donne au professionnel une confiance qu'il paiera le jour
         où il comptera sur un résumé qui n'est jamais parti. Le produit
         nomme ailleurs ses limites — il les nomme ici aussi. */
      { label: '✉ Préparer le résumé du jour', act: function () {
          store.log('Résumé du jour préparé', S.identity.email || 'adresse professionnelle');
          flash('Résumé prêt pour ' + (S.identity.email || 'votre adresse')
            + " — l'envoi par email n'est pas encore branché."); } },
    ...
    return '<div class="stack limit-760">' +
      ...
      (store.can('knowledge')
        ...
        '<form class="faq-form" id="faq-form" hidden>' +
          ...
          '<p class="field-error" id="faq-error" role="alert" hidden></p>' +
  ...
  function accountPlan() {
    ...
    return (window.ALLY_TEAM ? window.ALLY_TEAM.view() : '') +
      ...
      '<div class="plan-card">' +
        ...
        /* Ces trois choix étaient « Essai gratuit », le libellé du profil
           métier, et « Cabinet illimité » — trois noms hérités d'un prototype
           et absents du catalogue. Les choisir ne changeait que l'étiquette :
           l'identifiant de formule ne bougeait pas, donc ni le quota, ni le
           prix, ni les capacités. Quelqu'un se croyait passé en « illimité »
           pendant qu'Ally lui appliquait les quatre cents appels de la
           formule Cabinet. Une interface peut se tromper ; elle ne doit pas
           affirmer sur le contrat quelque chose de faux.

           On propose donc les formules qui existent, et les choisir les
           applique vraiment. */
        '<div class="plan-choices" id="plan-choices" hidden>' +
          window.ALLY_PLANS.map(function (formule) {
            return '<button type="button" class="choice" data-plan="' + esc(formule.id) + '"' +
              ' aria-pressed="' + (S.planId === formule.id) + '">' + esc(formule.name) +
              ' <span class="choice-count">' + formule.quota.calls + ' appels</span></button>';
          }).join('') +
        '</div>' +
      '</div>' +
  ...
  function accountAlerts() {
    return '<div class="stack limit-640">' +
      '<div class="card"><p class="card-title">Canaux d\'alerte pour les urgences</p>' +
        ...
        /* Trois interrupteurs qui enregistrent une préférence et rien d'autre :
           ni SMS, ni notification système, ni email ne partent aujourd'hui. Le
           seul canal qui fonctionne vraiment est le transfert d'appel, et il se
           pose depuis Téléphonie. Le dire vaut mieux que de laisser quelqu'un
           attendre un SMS un dimanche soir. */
        '<p class="note note-sep" style="margin-top:16px">Ces trois canaux sont ' +
          'enregistrés mais pas encore actifs : SMS, notification et email attendent ' +
          'leurs services d\'envoi. Le canal qui fonctionne dès aujourd\'hui est le ' +
          'transfert d\'appel sur votre portable, à poser depuis ' +
          '<button type="button" class="btn-link" data-jump="telephony">Téléphonie</button>.</p>' +
      '</div>' +
      ...
      '<div class="card"><p class="card-title">Ce que vous recevrez</p>' +
        ...
        '<div class="transcript-actions" style="margin-top:16px">' +
          '<button type="button" class="btn btn-ghost btn-sm" id="digest-send">' +
            'Me le préparer maintenant</button>' +
          /* « Envoyé chaque soir à 19 h » décrivait un envoi qui n'a jamais
             lieu : aucun service d'emails n'est branché. Un réglage qui
             promet une habitude qu'il ne tient pas se découvre le jour où
             l'on comptait dessus. */
          '<span class="row-meta">Prévu pour ' + esc(S.identity.email || 'votre adresse') +
            (S.summaryFreq === 'daily' ? ' chaque soir à 19 h' : ' chaque vendredi à 18 h') + '</span>' +
        '</div>' +
        '<p class="note note-sep" style="margin-top:14px">L\'envoi par email attend ' +
          'le branchement du service d\'envoi. En attendant, le résumé se lit ici et ' +
          's\'exporte depuis « Aujourd\'hui ».</p>' +
      '</div></div>';
  }
  ...
  function renderPanel() {
    ...
    if (faqOpen && faqForm) {
      ...
      /* Le formulaire refusait en silence : on tapait la question, on cliquait
         « Ajouter », rien ne se passait et rien ne disait pourquoi. */
      var faqErreur = panel.querySelector('#faq-error');
      function direFaq(message, champ) {
        if (faqErreur) { faqErreur.textContent = message; faqErreur.hidden = !message; }
        if (champ) { champ.setAttribute('aria-invalid', 'true'); champ.focus(); }
      }
      ['faq-q', 'faq-a'].forEach(function (id) {
        var champ = panel.querySelector('#' + id);
        if (!champ) return;
        champ.addEventListener('input', function () {
          champ.removeAttribute('aria-invalid');
          if (faqErreur) faqErreur.hidden = true;
        });
      });

      faqForm.addEventListener('submit', function (event) {
        event.preventDefault();
        var champQ = document.getElementById('faq-q');
        var champA = document.getElementById('faq-a');
        var q = champQ.value.trim();
        var a = champA.value.trim();

        if (!q) {
          direFaq('La question, telle qu\'un client la pose au téléphone.', champQ);
          return;
        }
        if (!a) {
          direFaq('Ce qu\'Ally répondra, mot pour mot. C\'est cette phrase qu\'entendra votre '
            + P().clientWord + '.', champA);
          return;
        }

        /* Deux fiches à la même question, c'est deux réponses possibles à un
           client, et le moteur en choisit une. Il vaut mieux modifier celle
           qui existe que d'en poser une seconde à côté. */
        var normalise = function (t) {
          return t.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')
            .replace(/[^a-z0-9]+/g, ' ').trim();
        };
        var jumelle = D().faq.filter(function (f) { return normalise(f.q) === normalise(q); })[0];
        if (jumelle) {
          direFaq('Cette question a déjà une réponse : « ' + jumelle.a.slice(0, 60) +
            (jumelle.a.length > 60 ? '…' : '') + ' ». Supprimez-la d\'abord si vous voulez la remplacer.',
            champQ);
          return;
        }
        ...
        store.save();
        renderPanel();
        flash('Ally sait maintenant répondre à « ' + q + ' ».');
      });
    }
    /* Supprimer une fiche change ce qu'Ally dit à un client, et se faisait
       d'un clic sans confirmation ni retour en arrière. Annuler un
       rendez-vous en demande une ; retirer une réponse au public mérite la
       même retenue. Deux temps, et l'armement retombe seul. */
    panel.querySelectorAll('[data-faq-del]').forEach(function (button) {
      ...
      button.addEventListener('click', function () {
        ...
        if (button.getAttribute('data-armed') !== '1') {
          ...
          button.textContent = 'Confirmer — Ally ne saura plus répondre';
        ...
        var fiche = D().faq.filter(function (f) { return f.id === id; })[0];
        D().faq = D().faq.filter(function (f) { return f.id !== id; });
        store.log('Retrait de la base de connaissances', fiche ? fiche.q : '');
        store.save();
        renderPanel();
        flash(fiche
          ? 'Ally ne répondra plus à « ' + fiche.q +' ».'
          : 'Fiche retirée.');
      });
    });
    ...
    if (wipeBtn) {
      wipeBtn.addEventListener('click', function () {
        if (wipeBtn.getAttribute('data-armed') !== '1') {
          ...
          wipeBtn.textContent = 'Confirmer — le compte sera supprimé';
        ...
        /* Sans serveur, l'effacement était incomplet : store.reset() vide les
           données de travail, et c'est tout. Le compte restait dans
           l'annuaire du navigateur avec son adresse et son mot de passe
           haché, et l'identité — nom, cabinet — revenait au rechargement,
           puisque c'est l'annuaire qui en fait autorité.

           Le bouton dit « Supprimer toutes mes données ». Il ne peut pas en
           laisser derrière lui : le droit à l'effacement porte sur le compte,
           pas seulement sur son contenu. accounts.remove fait le reste — il
           retire l'utilisateur, ferme sa session et efface sa clé de
           stockage nominative.

           Le compte de démonstration anonyme n'a rien à supprimer : il
           repart simplement à neuf. */
        var moi = window.ALLY_ACCOUNTS && window.ALLY_ACCOUNTS.currentId();
        store.reset();
        if (moi) window.ALLY_ACCOUNTS.remove(moi);
        window.ALLY_RESTART();
      });
    }
    ...
    if (digestSend) {
      digestSend.addEventListener('click', function () {
        store.log('Résumé préparé à la demande', S.identity.email || 'adresse professionnelle');
        flash('Résumé prêt pour ' + (S.identity.email || 'votre adresse')
          + " — l'envoi par email n'est pas encore branché.");
      });
    }
    ...
    if (chatForm) {
      ...
      chatForm.addEventListener('submit', function (event) {
        ...
        /* Une rangée de propositions sous la dernière réponse : une seule à
           la fois, la précédente s'efface. */
        function propositions(libelles, faire) {
          var vieille = log.querySelector('.chat-follow');
          ...
          libelles.forEach(function (libelle) {
            ...
            chip.addEventListener('click', function () { faire(libelle, row); });
        ...
        /* Le chat annonçait l'action et ne la faisait pas.

           « Annule le rendez-vous de Mme Martin » répondait « C'est annulé,
           j'ai prévenu Mme Martin, le créneau est libéré » — et le rendez-vous
           restait à l'agenda. Aucun appel à apply() n'existait de ce côté :
           seule la fenêtre vocale exécutait. La carte promet pourtant « même
           moteur que la commande vocale : elle répond pareil » ; c'était vrai
           des mots, faux des actes. */
        function executer(res, ordre) {
          if (res.apply) res.apply();
          store.log(ordre, res.kind === 'action' ? 'Traité par Ally' : 'Répondu');
          /* Pas de renderPanel() : il reconstruirait la carte et effacerait la
             conversation en cours. Le compte des onglets et l'en-tête, eux,
             doivent suivre. */
          renderNav();
          renderChrome();
        }
        ...
        var doitConfirmer = result.kind === 'action' && !!result.confirm &&
          (S.confirmLevel === 'always' || (S.confirmLevel === 'sensitive' && result.sensitive));
        var answer = doitConfirmer ? result.confirm : result.reply;

        if (result.kind === 'action' && !doitConfirmer) executer(result, text);

        window.setTimeout(function () {
          bubble('in', answer, result.detail);
          if (S.chatSpeaks) window.ALLY_VOICE.speak(answer, store.voiceOptions());

          /* Une confirmation demandée doit pouvoir être donnée. La question
             s'affichait sans aucun moyen de répondre oui : l'ordre restait en
             suspens pour toujours. */
          if (doitConfirmer) {
            propositions(['Oui, faites-le', 'Non, laissez'], function (libelle, row) {
              row.remove();
              if (/^Oui/.test(libelle)) {
                executer(result, text);
                bubble('in', result.reply, result.detail);
                propositions(result.follow, relancer);
              } else {
                store.log(text, 'Abandonné avant exécution');
                bubble('in', 'C\'est noté, je ne fais rien.');
              }
            });
            return;
          }
